...
class _PostPropertyScreenState extends State<PostPropertyScreen> {
  ...
  void initState() {
    ...
    if (widget.propertyToEdit == null) {
      _titleCtrl.addListener(_autoGenDesc);
      _carpetAreaCtrl.addListener(_autoGenDesc);
    }
  }

  @override
  void dispose() {
    _titleCtrl.removeListener(_autoGenDesc);
    _carpetAreaCtrl.removeListener(_autoGenDesc);
    _titleCtrl.dispose();
    _descCtrl.dispose();
    _priceCtrl.dispose();
  ...
  void _autoGenDesc() {
    if (!mounted) return;
    if (_descCtrl.text.isNotEmpty && !_descAutoGenerated) return;
    final d = _generateDescription();
    if (d.isEmpty) return;
    _descCtrl.text = d;
    _descAutoGenerated = true;
  }
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                    ...
                    child: Column(
                      ...
                      children: [
                        TextFormField(
                          ...
                          onChanged: (_) => _descAutoGenerated = false,
                        ),
                        const SizedBox(height: 4),
                        GestureDetector(
                          onTap: () => setState(() {
                            _descCtrl.text = _generateDescription();
                            _descAutoGenerated = true;
                          }),
                          child: const Padding(
                  ...
                  _chipSelector(_rentSaleOptions, rentOrSale, (v) {
                    setState(() => rentOrSale = v);
                    _autoGenDesc();
                  }),
